...
type ChatController struct {
	ChatService *api_service.ChatService
}

func NewChatController(service *api_service.ChatService) *ChatController {
...
func (controller *ChatController) VerifyContactHandler(c *fiber.Ctx) error {
	...
	res, err := controller.ChatService.VerifyContact(claims["username"].(string))
	if err != nil {
...
func (controller *ChatController) ChatHistoryHandler(c *fiber.Ctx) error {
	...
	res, err := controller.ChatService.ChatHistory(username1, username2)
	if err != nil {
		return c.Status(fiber.StatusInternalServerError).JSON(api_structure.ErrorResponse{
...
func (controller *ChatController) ContactListHandler(c *fiber.Ctx) error {
	...
	res, err := controller.ChatService.ContactList(claims["username"].(string))
	if err != nil {
		return c.Status(fiber.StatusInternalServerError).JSON(api_structure.ErrorResponse{
			Type:    "Chat List Data",
			Message: err.Error(),
